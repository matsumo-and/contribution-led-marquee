 ...
 */
import { getCharBitmap, getCharWidth, BITMAP_CHAR_HEIGHT } from './bitmapFont';
...
export function renderText(text: string, options: TextRenderOptions = {}): RenderedText {
  ...
  const height = BITMAP_CHAR_HEIGHT;

  // Get bitmaps for all characters
  const charBitmaps: number[][][] = [];
  let totalWidth = 0;
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (bitmap) {
      ...
      const spaceBitmap = getCharBitmap(' ');
      if (spaceBitmap) {
